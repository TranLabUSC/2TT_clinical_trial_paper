...
plotPathwayUMAP <- function(gmt_file, pathway_name, seurat_obj, survival_data, file_name, resection_type) {
  ...
  # Pre-determine the appropriate axis limits
  Y_MIN <- -12 # example value, adjust as needed
  Y_MAX <- 5  # example value, adjust as needed
  X_MIN <- -3 # example value, adjust as needed
  X_MAX <- 6  # example value, adjust as needed
  ...
  for (tp in timepoints) {
    ...
    if (length(patient_tp_cells) > 0) {
      ...
      p <- FeaturePlot(patient_seurat_obj, features = "pathway_avg_expression", cells = patient_tp_cells) &
        scale_colour_gradientn(limits = c(-3, 3), colours = rev(brewer.pal(n = 7, name = "RdBu"))) &
      ...
      plot_list[[plot_counter]] <- p + coord_flip() +
        xlim(X_MIN, X_MAX) + ylim(Y_MIN, Y_MAX) # Set fixed axis limits
    } else {
...
resection_types = c("Biopsy Only", "Maximal Resection")

for (pathway_name in pathways){
  print(pathway_name)
  for (resection_type in resection_types){
    file_name <- paste0("/project/dtran642_927/Data/Collaborator/DJ/scRNAseq/latest_big_sequencing_projects/2TT/Analysis/Aggregate_ge_only/Nextflow/Aggregated/outs/NETZEN_analysis/Finale/GBM/output_file_", pathway_name,  "_", resection_type,"_DC_cells.pdf")
    plotPathwayUMAP(gmt_file, pathway_name, seurat_object_subset, survival_data, file_name, resection_type)
  }
}