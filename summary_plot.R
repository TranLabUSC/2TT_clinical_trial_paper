...
make_cox_fit = function(cox,
                        ...
                        survival_measure,
                        compared_pair,
                        fit_model=c("clinical_only", "uni", "clinical_uni", "clinical_resection", "clinical_resection_uni", "clinical_Maxresection", "clinical_Maxresection_uni"),
                        variable_name=NULL)
{
  cox_fit <- survfit(cox)
  #When there is NA value, replaced by 0 for lower, 1 for upper confidence as autoplot with ignore NA, make a false impression of tied error
  cox_fit$upper[is.na(cox_fit$upper)] <- 1
  cox_fit$lower[is.na(cox_fit$lower)] <- 0
  p_cox_fit = autoplot(cox_fit)
  concordance = summary(cox)$concordance [["C"]]
  coeffs = summary(cox)$coefficients
  print(coeffs)
  write.table(coeffs, file = paste0(filename, "_", fit_model,  ".cox_fit_coeff.txt"))
  print(summary(cox)$waldtest)
  plot_label = paste(
    survival_measure,
    " Cox fit model\n",
    compared_pair[[1]],
  ...
  if (fit_model %in% c("clinical_only","clinical_resection", "clinical_Maxresection" ))
  {
    plot_label = paste( plot_label, "\n", fit_model, "\n\n\n" )
  }else 
  {
    plot_label = paste( plot_label, "\n", fit_model, "\n",  variable_name, "p-val: ", round(coeffs["comparison_value", 5],3), "\n",variable_name, "HR: ", round(coeffs["comparison_value", 2],2))
    print(plot_label)
  }
  print(plot_label)
...
extract_survival_curve_data <- function(cox_model, label) {
  surv_fit <- survfit(cox_model)
  
  surv_fit$upper[is.na(surv_fit$upper)] <- 1
  surv_fit$lower[is.na(surv_fit$lower)] <- 0
  surv_data <- tidy(surv_fit)
  surv_data$group <- label # Add a label to differentiate the curves
  return(surv_data)
}


make_overlay_plot= function(data_clinical_only, data_multi)
{
combined_data <- rbind( data_clinical_only, data_multi)
print("combined data")
print(combined_data)
...
overlay_plot <- ggplot(combined_data, aes(x = time, y = estimate, group = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2) + # Add fill within aes()
  geom_step(aes(color = group)) +
  scale_fill_manual(values = c( "Clinical Only" = "red", "Multivariate" = "blue")) +
  scale_color_manual(values = c( "Clinical Only" = "red", "Multivariate" = "blue")) +
  labs(title = "Survival Curve by Group",
       x = "Time (months)",
...
make_comparison_value_low_high_fit_plot = function(comparison_df,
                                                   ...
                                                   plot_folder = "outs",
                                                   variable_name = "Shannon diversity",
                                                   variable_to_stratify="comparison_value")
{

  #print(comparison_df)
  comparison_value_median = median(comparison_df[,variable_to_stratify])
  comparison_df$comparison_value_group = sapply(comparison_df[,variable_to_stratify], function(x)
  {
  ...
  #print("comparison_df stratified")
  #print(comparison_df)
  ...
  p = autoplot(km_comparison_value_group_fit, conf.int = FALSE)
  p = p +   annotate(
    "text",
    label = p_label,
    y = 0.95,
                               ...
                               clinical_formula = 'Age + Sex + MGMT.methylation + IDH.1.mutation')
{
  survival_object = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~ ", clinical_formula," + comparison_value"
    )
  )
  cox  = coxph(survival_object, data = comparison_df, na.action = na.omit)
  
  survival_object_clinical_only = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~  ", clinical_formula
    )
  )
  cox_clinical_only  = coxph(survival_object_clinical_only, data = comparison_df, na.action = na.omit)
  
  survival_object_uni           = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~  ", "comparison_value"
    )
  )
  cox_uni  = coxph(survival_object_uni, data = comparison_df, na.action = na.omit)

  

  survival_object_clinical_resection           = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~  ", clinical_formula, " + resection"
    )
  )
  cox_clinical_resection  = coxph(survival_object_clinical_resection, data = comparison_df, na.action = na.omit)
  
  
  survival_object_clinical_resection_comparison_val           = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~  ", clinical_formula, " + resection + comparison_value"
    )
  )
  cox_clinical_resection_comparison_val  = coxph(survival_object_clinical_resection_comparison_val, data = comparison_df, na.action = na.omit)
  
  
    
  survival_object_clinical_Maxresection           = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~  ", clinical_formula, " + Max_resection"
    )
  )
  cox_clinical_Maxresection  = coxph(survival_object_clinical_Maxresection, data = comparison_df, na.action = na.omit)
  
  
  survival_object_clinical_Maxresection_comparison_val           = as.formula(
    paste0(
      "Surv(comparison_df[, '",
      survival_measure,
      "'], comparison_df$Dead)  ~  ", clinical_formula, " + Max_resection + comparison_value"
    )
  )
  cox_clinical_Maxresection_comparison_val  = coxph(survival_object_clinical_Maxresection_comparison_val, data = comparison_df, na.action = na.omit)
  ...
  cox_fit_clinical_only_filename = paste0(
    plot_folder,
    "/",
    "coxfit_clinical_only",
    survival_measure,
    "_vs_relative_",
    variable_name,
    "_",
  ...
  cox_fit_plot_clinical_only = tryCatch({
    make_cox_fit(
      cox = cox_clinical_only,
      filename = cox_fit_clinical_only_filename,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      fit_model =  "clinical_only"
    )
  },
  error = function(x)
  {
  ...
  cox_fit_plot_uni= tryCatch({
    make_cox_fit(
      cox = cox_uni,
      filename = cox_uni_filename,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      fit_model = "uni"
    )
  ...
  cox_fit_plot_clinical_resection= tryCatch({
    make_cox_fit(
      cox = cox_clinical_resection,
      filename = cox_uni_filename,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      fit_model = "clinical_resection"
    )
  },
  error = function(x)
  {
  ...
  cox_fit_plot_clinical_resection_comparison_val = tryCatch({
    make_cox_fit(
      cox = cox_clinical_resection_comparison_val,
      filename = cox_uni_filename,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      fit_model = "clinical_resection_uni"
    )
  },
  error = function(x)
  {
  ...
  cox_fit_plot_clinical_Maxresection= tryCatch({
    make_cox_fit(
      cox = cox_clinical_Maxresection,
      filename = cox_uni_filename,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      fit_model = "clinical_Maxresection"
    )
  },
  error = function(x)
  {
  ...
  cox_fit_plot_clinical_Maxresection_comparison_val = tryCatch({
    make_cox_fit(
      cox = cox_clinical_Maxresection_comparison_val,
      filename = cox_uni_filename,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      fit_model = "clinical_Maxresection_uni"
    )
  },
  error = function(x)
  {
  ...
  risk_score_cox <- predict(cox, type = "lp")
  comparison_df$risk_score_cox = risk_score_cox
  km_plot_coxph = tryCatch({
    make_comparison_value_low_high_fit_plot(
      comparison_df = comparison_df ,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      plot_folder = plot_folder,
      variable_name = "risk_score_cox",
      variable_to_stratify="risk_score_cox"
    )
  },
  error = function(x)
  {
  ...
  risk_score_cox_clinical_only <- predict(cox_clinical_only, type = "lp")
  comparison_df$risk_score_cox_clinical_only = risk_score_cox_clinical_only
  
  km_plot_coxph_clinical_only = tryCatch({
    make_comparison_value_low_high_fit_plot(
      comparison_df = comparison_df ,
      survival_measure = survival_measure,
      compared_pair = compared_pair,
      plot_folder = plot_folder,
      variable_name = "risk_score_cox clinical only",
      variable_to_stratify="risk_score_cox_clinical_only"
    )
  },
  error = function(x)
  {
  ...
  data_clinical_only <- extract_survival_curve_data(cox_clinical_only, "Clinical Only")
  data_multi <- extract_survival_curve_data(cox, "Multivariate")
  
  overlay_plots = make_overlay_plot(data_clinical_only,data_multi )
  
  
  
  filename = paste0(plot_folder, "/", survival_measure,"_overlay_cox_fit_plots.pdf")
  ...
  plot_list = list(survival_dot_plot,
                   cox_fit_plot_uni,
                   cox_fit_plot_clinical_only,
                   cox_fit_plot,
                   overlay_plots,
                   cox_fit_plot_clinical_resection,
                   cox_fit_plot_clinical_resection_comparison_val,
                   cox_fit_plot_clinical_Maxresection,
                   cox_fit_plot_clinical_Maxresection_comparison_val,
                   km_plot, 
                   km_plot_coxph_clinical_only,
                   km_plot_coxph )
  
  #plot_list = list( cox_fit_plot_clinical_only, cox_fit_plot,  km_plot, km_plot_coxph_clinical_only,  km_plot_coxph )
  ...
  plot_width = 20 * length(plot_list)
  plot_height = 20
  cairo_pdf(file =  filename ,
            width = plot_width,
            height = plot_height)
